...
품질 반복: 실제 글 3~5편을 few-shot으로 넣는 것은 SP3(스타일 프로필)에서 결정. SP2에서는 메모리 항목으로 대체.
...
- **계획 패널 흐름(사용자 결정 2026-08-29)**: §6의 `plan`은 `{titleCandidates, outline, tone}` 객체였으나 **마크다운 문자열 하나**(`TurnResponse.plan: String?`)로 바뀌었다. 형식은 `# 확정 제목` / `다른 제목: …` / `## 글 구성`(번호 목록) / `## 말투와 분위기`이고 프롬프트 `06_conversation.md`에 적혀 있다(관리자 화면에서 편집 가능). 계획은 채팅 말풍선(`PlanCard`, 삭제됨)이 아니라 **오른쪽 패널**(`ui/chat/components/PlanPanel.kt` + 의존성 없는 `MarkdownLite.kt`)에 그리고, 채팅 목록에는 "계획을 오른쪽에 정리했어요 · 보기" 한 줄만 남는다. 제목 고르기 단계는 없다 — 피드백을 주면 모델이 **계획 전체**를 다시 내고(`ChatContext.currentPlan`으로 이전 계획을 함께 보낸다), 패널이 갱신된다. 계획이 있고 아직 초안이 없는 동안 입력창 위에 고정 버튼 "이대로 초안 써 줘"가 보이며(칩 자동 추가 로직은 제거), 누르면 초안 턴이 돌고 그 자리를 `PublishPanel`(스마트에디터)이 가져간다. 세션 이름은 계획 첫 줄 `# 제목`에서 딴다(사용자가 바꾼 이름은 그대로 둔다).
- **협찬 표기는 앱이 다루지 않음(사용자 결정 2026-08-29)**: 프롬프트에서 협찬·대가·공정위 관련 항목을 모두 뺐다. 협찬 문구가 필요하면 사용자가 검토 단계에서 직접 추가한다.
- **계획 전 사실 확인 루프(사용자 결정 2026-08-29)**: 계획을 내기 전, 글에 꼭 필요한 사실이 비어 있으면 모델이 `question` 턴으로 한 번에 1~2개씩 묻는다(세션당 최대 4턴, `ChatContext.questionRounds`/`maxQuestionRounds`). 질문 턴 = `plan == null && question != null` 이고 `readyToDraft` 는 강제로 false 다. 4회에 닿으면 `ConversationEngine` 이 "더 묻지 말고 지금 정보로 계획을 내라"는 지시로 바꾼다.
- **초안 품질 게이트(사용자 결정 2026-08-29)**: 초안·수정 턴의 `post` 는 에디터에 들어가기 전에 로컬 점검을 거친다 — 본문 글자 수(공백·제목 제외)가 `ModelPolicy.targetLength` ±10% 밖이거나, 엔진이 사진 누락·중복을 보정했으면 `ChatUiState.draftGate` 로 보류하고 채팅에 카드를 띄운다("이대로 넣기" / "고쳐 달라고 하기"). 모델을 다시 부르지 않으며, 게이트는 턴마다 새로 계산하는 메모리 상태다.
- **턴별 thinking(사용자 결정 2026-08-29)**: `GGenerationConfig.thinkingConfig` 로 초안·수정 턴은 `"high"`, 질문·계획·피드백 턴은 `"low"` 를 요청한다. 400 응답 메시지에 `thinking` 이 들어 있으면 그 필드만 빼고 같은 pick 으로 한 번 더 보낸다(스키마 드롭과 독립된 플래그).
- **시작 화면을 채팅으로(사용자 결정 2026-08-29)**: `Routes.Home`·`ui/home/*` 를 없애고 `Routes.Chat(sessionId = null)` 을 시작 화면으로 삼았다. 메시지가 없는 "새 글" 은 큰 제목 + 가운데 입력창(`Composer(hero = true)`)으로 보이고, 대화는 첫 `send`/`attachPhotos` 때 `ChatViewModel.ensureSession` 이 만든다(빈 대화가 목록에 쌓이던 문제 해결). 홈이 하던 업데이트 배너(FR-12)는 `ChatViewModel` 로 옮겼고, 세션 없는 옛 pending 작업 배너는 없앴다. 설정 톱니는 대화 기록 사이드바 맨 아래.
- **발행 기록 UI 제거(사용자 결정 2026-08-29)**: `Routes.History`·`ui/history/*` 를 삭제했다. `publish_history` 테이블과 발행 후 저장 로직(`HistoryRepository`, `PublishStateMachine` 의 `SavePublished`)은 스키마 변경을 피하려고 그대로 둔다 — 지금은 아무 화면도 읽지 않는다.
