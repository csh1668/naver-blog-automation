...
- 상태: **사용자 검토 전 초안.** 아래 "결정 필요" 항목은 제가 가정한 값이며, 사용자가 바꾸면 그대로 반영한다.
...
│  └─ MemoryExtractor.kt       세션 종료·발행 시 LLM으로 새 기억 후보 추출 → 사용자 확인
├─ chat/
...
- `ApiKeyParser.parse(text): List<String>`: 줄바꿈·쉼표·공백·세미콜론으로 분리 → 앞뒤 따옴표/`key=` 접두 제거 → Gemini 키 형태(`^AIza[0-9A-Za-z_-]{30,}$`)만 채택 → 중복 제거, 이미 등록된 키 제외. 형태가 다른 토큰은 "키처럼 보이지 않아요" 칩으로 표시(등록 안 함).
...
| 도구 | 시그니처 | 구현 | 제한 |
|---|---|---|---|
| `web_search` | `(query: string) → [{title, url, snippet}]` (최대 5) | `WebResearchTool`: 숨은 WebView가 `https://search.naver.com/search.naver?where=view&query=…`(블로그/뷰 탭) 또는 `https://www.google.com/search?q=…`를 로드 → `research_extract.js`가 결과 카드에서 제목/URL/요약을 추출 | 턴당 2회, 8초 제한 |
...
| `remember` | `(kind: STYLE|PREFERENCE|FACT|EXPRESSION, text: string) → ok` | `MemoryStore`에 **후보**로 저장하고 채팅에 "기억해 둘까요?" 칩(예/아니요) 표시. 사용자가 예를 눌러야 활성 | 턴당 1회 |
...
`PromptBuilder`가 매 턴 조립하는 시스템 프롬프트의 섹션(순서 고정, 각 섹션은 리소스 파일로 관리해 수정 가능):

1. **역할**: "당신은 한국어 네이버 블로그 글쓰기 도우미. 60대 여성 블로거(사용자)의 목소리로 글을 쓴다. 대화는 짧고 다정하게, 질문은 한 번에 하나."
...
5. **글 구조 규칙**:
   ...
   - 길이: 기본 900~1,400자(사용자가 "짧게/길게" 하면 ±40%).
...
- **ChatScreen**: 가로 2단(왼쪽 채팅, 오른쪽 `PublishPanel` 50%·최소 520dp, 슬라이드 인), 세로는 패널이 전체 화면 시트. 상단 얇은 바: 세션 제목(첫 제목 후보 또는 "새 글") + 패널 접기/펼치기.
...
- **관리자**: PinGate → Settings 목록(API 키, 모델, 메모리, 검색 도구 토글, 실패 로그, 네이버 로그아웃). MemoryScreen: 종류별 목록, 스위치로 비활성, 스와이프 삭제 없음(ConfirmSheet).
...
| 항목 | 가정 | 대안 |
|---|---|---|
| 검색 엔진 | 네이버 검색(뷰 탭) 우선, 실패 시 구글 | 구글만 |
