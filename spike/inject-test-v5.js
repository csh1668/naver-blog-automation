// [스파이크용 throwaway] v5: uploadImagesFromFiles 가 Promise 배열을 반환하므로 Promise.all 로 대기
// 사용법: 글쓰기 페이지 F12 → Console 붙여넣기 → Enter
// 결과: 콘솔 마지막 줄 "RESULT_JSON {...}" 우클릭 → Copy string contents → spike/inject-result-v5.json 에 저장
(async () => {
  ...
  try {
    ...
    const files = [await makeFile('SPIKE v5 image 1', '#3182f6'), await makeFile('SPIKE v5 image 2', '#f04452')];
